...
* **BEM event** is the event, generated by block itself. It allows to organize API for [interaction with the block](#ibc). BEM events usually handle the block instance, that tracks the states of other blocks generating events. 

When planning interface architecture, consider that DOM events should only be used in *inner* block procedures. 
