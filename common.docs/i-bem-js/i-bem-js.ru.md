...
Planning interface architecture, one needs to consider, that DOM events should only be used in block instance methods / *внутренних* процедурах блока. 
...
BEM and DOM events handling can be **делегирована** to the container/контейнеру
...
некоторые из дочерних узлов еще не существовали. In this case container is a point of events handling, triggered in any of the child nodes, so it will hanlde events for all child nodes, even if they will be loaded. 

For example, блок меню menu block can contain nested blocks (or elements, this depends on the specific block implementation) — menu items. 
Обработку кликов click event handling on menu items it makes sence to delegate to the menu block itself. 
This, first of all, allows to save the overhead of adding events затраты ресурсов на
подписку на события (it's cheaper to add one event listener to a container, than many events listeners to the elements). 
Во-вторых, это дает возможность более
...
Second, you can change menu contents: add and remove menu items without
adding event listener for added items or remove listeners from deleted items.
